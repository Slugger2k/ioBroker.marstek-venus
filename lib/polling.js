...
const Polling = {
	...
	async pollSlow() {
		if (this._slowPollingInProgress) {
			this.log.debug("Long-Polling already in progress, skipping");
			return;
		}
	...
	async poll() {
		if (this._pollingInProgress) {
			this.log.debug("Standard-Polling cycle already in progress, skipping");
			return;
		}
		...
		try {
			...
			if (this.config.enableBatteryStatus !== false) {
				pollTasks.push(this.pollBatteryStatus());
			}
			if (this.config.enableEMStatus !== false) {
				pollTasks.push(this.pollEMStatus());
			}
			if (this.config.enableModeStatus !== false) {
				pollTasks.push(this.pollModeStatus());
			}
			if (this.config.enablePVStatus !== false && this.hasPVSupport()) {
				pollTasks.push(this.pollPVStatus());
	...
	async pollWifiStatus() {
		try {
			...
			if (result.sta_gate !== undefined && result.sta_gate !== null) {
				await this.setStateChangedAsync("network.gateway", { val: result.sta_gate, ack: true });
			}
			if (result.sta_mask !== undefined && result.sta_mask !== null) {
				await this.setStateChangedAsync("network.mask", { val: result.sta_mask, ack: true });
			}
			if (result.sta_dns !== undefined && result.sta_dns !== null) {
				await this.setStateChangedAsync("network.dns", { val: result.sta_dns, ack: true });
			}
	...
	async pollEMStatus() {
		...
		if (result.input_energy !== undefined && result.input_energy !== null) {
			await this.setStateChangedAsync("energymeter.inputTotal", {
				val: Math.round(result.input_energy * 0.1),
				ack: true,
			});
		}
		if (result.output_energy !== undefined && result.output_energy !== null) {
			await this.setStateChangedAsync("energymeter.outputTotal", {
				val: Math.round(result.output_energy * 0.1),
				ack: true,
			});
		}
	},
